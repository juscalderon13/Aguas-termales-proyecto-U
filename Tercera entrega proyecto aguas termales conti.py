# Proyecto Aguas termales version 1.02
...
def SubmodRegisVisit():
    ...
    #REVISION DE FECHA Y CREACION DE FORMATO DD/MM/AAAA
    fecha = date.today() 
    fecha = str(fecha)
    nuevafecha = fecha[8:]+"-"+fecha[5:7]+"-"+fecha[0:4]

    #creacion de archivo para la fecha de registro de usuarios
    file = open(f"{nuevafecha}RV.txt","a")
    ...
    if edad < 18:
        categoria = "NIÑO"
        file.write("1") #Ingresar valor de 1 al archivo de esa fecha
        file.write("\n")
    elif (edad >= 18) and (edad < 65):
        categoria = "ADULTO"
        file.write("2") #Ingresar valor de 2 al archivo de esa fecha
        file.write("\n")
    elif edad >= 65:
        categoria = "ADULTO MAYOR"
        file.write("3") #Ingresar valor de 3 al archivo de esa fecha
        file.write("\n")
    print("")
...
""")
    ...
    file.close() # cerrar archivo de registro
...
def SubModulAforo():
    ...
    fechaInp = input() #Fecha ingresada por el usuario en formato (DD/MM/AAAA)
    fechaUser = fechaInp #Clonar valor para alteracion de formato de fecha
    fechaUser = fechaUser[0:2] + "-" + fechaUser[3:5] + "-" + fechaUser[6:] + "RV.txt" #pasar de formato (DD/MM/AAA) a (DD-MM-AAAA)RV.txt
    validacion = os.path.isfile(f'{fechaUser}') #validamos si el archivo existe

    if validacion != True: #si el archivo no existe le informa que no historial de esa fecha
        print("**No hay historial de aforo para esta fecha, por favor intente otra fecha**")
    
    else: #si el archivo existe solicitar los datos del archivo
        file = open(f"{fechaUser}","r") #apertura de archivo
        datos = file.read() #lectura de archivo
        aforo = datos.split("\n") #separacion de datos mediante enters o \n
        aforo.pop() #eliminar el ultimo enter porque esta vacio
        file.close() #cerrar el archivo
        for i in aforo: #pasar los datos a enteros uno por uno y agregarlos a aforoint
            i = int(i)
            aforoint.append(i)
        
        for i in aforoint: #enviar a diferentes listas de tipo de cliente acorde al valor
            if i == 1:
                nino.append(i)
            elif i == 2:
                adulto.append(i)
        ...
        #total de aforo por dia contando la cantidad de valores en lista
        totalAfor = len(aforoint)
        nino = len(nino)
        adulto = len(adulto)
        adultoMa = len(adultoMa)
